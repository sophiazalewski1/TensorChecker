...
class Context(dict):
    def __str__(self):
        items = [
            f"'{key}': {value if isinstance(value, TensorType) else value}"
    ...
    if isinstance(expr, ast.BinOp):
        ...
        if isinstance(left, TensorType) or isinstance(right, TensorType):
            ...
            if isinstance(expr.op, ast.Add) or isinstance(expr.op, ast.Sub):
                ...
                if (
                    isinstance(left, TensorType)
                    and isinstance(right, TensorType)
                    and left.size == right.size
                ):
                    return left

                # Scalar Addition
                elif isinstance(left, TensorType) and left.size == [1]:
                    return right
                elif isinstance(left, TensorType) and not isinstance(right, TensorType):
                    return left
                elif isinstance(right, TensorType) and right.size == [1]:
                    return left
                elif isinstance(right, TensorType) and not isinstance(left, TensorType):
                    return right
            ...
            # Matrix Multiplication
                ...
                if isinstance(left, TensorType) and isinstance(right, TensorType):
                    ...
                    print(
                        "both left and right side of matrix mul have to be tensor types"
                    )
                    return
